...
local function register_guard(def,spawnnodes,spawnchanche)
	...
	local defbox = 0.4
	minetest.register_entity("sauron:" .. def.name,{
		initial_properties = {
			name = def.name,
			hp_min = def.max_hp,
			hp_max = def.max_hp,
			...
			visual = "mesh",
			mesh = "sauron1.b3d",
			textures = {"doors_blank.png","sauron_weapon_1.png","sauron1.png"},
...
if bx["dragons:bat"] and bx["trolls:uruguay"]and bx["nazgul:witchy"] and bx["nazgul:ghost"] and bx["balrog:balrog"] and bx["necromancer:witchy"] and bx["lothwormdragons:seaserpent"] and bx["spiders:shelob"] and (math.random(1,1200) == 1) then

minetest.add_item(p1.object:getpos(), "default:the_one_ring")
p1.object:set_hp(0)
default_add_boss_kill_flag(p1,p2)
healthbar_the_ring_is_taken = true
