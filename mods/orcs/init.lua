...
--local default_human_orc_mob_weapon_table = {}
...
local function set_weapon(self,item,mob_level,mobtex,shieldtex)
	local def = (item or ItemStack("default:sword_steel")):get_definition()
...
set_weapon(self,default_human_orc_mob_weapon_table[--[[selfish.fighter_group or ]]"ORC"][sdata.weapon],selfish.mob_level,selfish.main_image,selfish.shield_image or "doors_blank.png")
self.wielded_weapon_index = sdata.weapon
else
local keylist = (default_human_orc_mob_weapon_table[selfish.fighter_group] or {KEYLIST = "STEELSWORD"}).KEYLIST
self.wielded_weapon_index = keylist[math.random(#keylist)]
set_weapon(self,(default_human_orc_mob_weapon_table[selfish.fighter_group] or {KEYLIST = "STEELSWORD",STEELSWORD = ItemStack("default:sword_steel")})[self.wielded_weapon_index],selfish.mob_level,selfish.main_image,selfish.shield_image or "doors_blank.png")
end
end
...
local function register_guard(def,spawnnodes,spawnchanche)
	...
	minetest.register_entity("orcs:" .. def.name,{
		initial_properties = {
			name = def.name,
			hp_min = def.max_hp,
			hp_max = def.max_hp,
			...
			textures = {"doors_blank.png","doors_blank","doors:blank"},--def.textures or {def.name .. "shield.png^swordf.png",def.name .. ".png"},
		...
		on_activate = function(self,sdata)
			...
			local sdata,first_time = self_first_spawn(sdata)
			if def.drops then
			...
			self.texture_index = sdata.tex_index or math.random(0,def.max_texture_index)
			local deftextures = def.textures or {}
			set_mob_weapon_acc_race(self,sdata,{main_image = deftextures[1] or  "lottmobs_".. def.texture_name.."_" ..self.texture_index.. ".png",
		...
		get_staticdata = function(self)
			return add_first_spawn_flag(minetest.write_json({weapon = self.wielded_weapon_index,lifetimer = self.lifetimer,tex_index=self.texture_index}))end,
...
regular_humanoid_spawn_registry("orcs:" .. def.name,spawnnodes,spawnchanche)
end
...
--[[minetest.register_abm({
	...
	action = function(p)
		...
		if minetest.get_node(p).name == "air" and minetest.get_node(p2).name == "air" then
			minetest.add_entity(p,"orcs:standarddwarf")
		end
	end,
})]]
...
register_guard({
	...
	name = "uruguay",
	drops = {"orc"},
	texture_name = "uruk_hai_armored",
	max_texture_index = 2,
	max_hp = 30,
	size = 1,
	armor = 30,
	speed = 3
},{"default:mordordust_mapgen","default:mordordust","default:angmarsnow"},1000)
register_guard({
	damage = 9,
	name = "uruk_hai_unarmored",
	drops = {"orc"},
	shield = noshield,
	texture_name = "uruk_hai",
	max_texture_index = 2,
	max_hp = 30,
	size = 1,
	speed = 3
},{"default:mordordust_mapgen","default:mordordust","default:angmarsnow"},1200)
register_guard({
	damage = 3,
	name = "standardorc",
	drops = {"orc"},
	...
	size = 0.9,
	speed = 3
},{"default:fangorndirt","default:mordordust","default:brownlandsdirt","default:mordordust_mapgen","default:angmarsnow"},500)
register_guard({
	damage = 2,
	name = "desertthug",
	drops = {"orc"},
	mesh = "character.b3d",
	shield = noshield,
	textures = {"desertthug.png"},
	max_hp = 25,
	size = 0.9,
	speed = 4
},{"default:desert_sand"},800)
--[[register_guard({
	damage = 12,
	name = "elf",
	drops = {"elf"},
