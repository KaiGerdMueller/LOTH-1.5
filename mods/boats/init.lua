...
function boat.on_activate(self, sd, dtime_s)
	...
	if sd and (string.sub(sd,1,7) == "player/") then
		...
		if sd.n then
			self.driver = minetest.get_player_by_name(sd.n)
		end
	...
	self.dunlending = dunlendingshplayers[name] and (dunlendingshlaw[name]<=0)
			self.driver:set_attach(self.object, "",
			{x = 0, y = 11, z = -3}, {x = 0, y = 0, z = 0})
			default.player_attached[sd.n] = true
		minetest.after(0.2, function()
			default.player_set_animation(self.driver, "sit" , 30)
		end)
		else
	self.hudwel = true
	self.orc = true
...
function boat.get_staticdata(self)
		local ret = ""
		if self.driver then
			ret = "player/" .. minetest.write_json({n = self.driver:get_player_name(),v = self.v})
		end
		return ret
end
