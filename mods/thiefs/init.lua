...
local function get_nearest_enemy(self,pos,radius,fucking_punchtest)
	...
	for _,entity in ipairs(minetest.get_objects_inside_radius(pos,radius)) do
		...
		if ((luaent and luaent.name == "__builtin:item") or (entity:is_player() and (not fucking_punchtest) and (dist < 8)))and (dist < min_dist) then
				min_dist = dist
				target = entity
		end
	end
...
local function register_guard(def,spawnnodes,spawnchanche)
	...
	local defbox = 0.4
	minetest.register_entity("thiefs:" .. def.name,{
		initial_properties = {
			name = def.name,
			hp_min = def.max_hp,
			hp_max = def.max_hp,
		...
		on_step = function(self, dtime)
			...
			if self.timer >= 1 then
				...
				if target then
				if string.find((target:get_luaentity() or {}).itemstring or "","default:the_one_ring") then
				healthbar_the_ring_is_taken=false
				healthbar_write_the_ring_is_taken()
				end
				if target:get_luaentity() and target:get_luaentity().name == "__builtin:item" then
				target:remove()
				else
...
register_guard({
	...
	size = 1,
	speed = 4.5
},{"default:gondordirt","default:ithiliendirt","default:rohandirt","default:dunlanddirt"},1200)
--[[register_guard({
	damage = 10,
	name = "copper",
	max_hp = 40,
