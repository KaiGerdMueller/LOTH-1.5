...
fire = {}
local function random_interpolate(a,b)
	if a > b then
...
vector.random_interpolate = function(a,b)
return {x=random_interpolate(a.x,b.x),y=random_interpolate(a.y,b.y),z=random_interpolate(a.z,b.z)}
end
local function add_particlespawner(def)
for i=1,def.amount do
minetest.after(random_interpolate(0,def.time),function()
minetest.add_particle({
	pos =vector.random_interpolate(def.maxpos,def.minpos),
	velocity = vector.random_interpolate(def.maxvel,def.minvel),
	acceleration = vector.random_interpolate(def.maxacc,def.minacc),
	expirationtime = random_interpolate(def.maxexptime,def.minexptime),
	size = random_interpolate(def.maxsize,def.minsize),
...
minetest.register_abm({
	...
	minvel = {x=-0.6, y=1, z=-0.6},
	maxvel = {x=0.6, y=1, z=0.6},
	minacc = {x=0, y=0, z=0},
	maxacc = {x=0, y=0, z=0},
	minexptime = 6,
	maxexptime = 10,
...
minetest.register_node("fire:blue_flame", {
	description = "Blue Flame",
	drawtype = "firelike",
	tiles = {
		{
			name = "bluefire_animated.png",
			animation = {
				type = "vertical_frames",
				aspect_w = 16,
				aspect_h = 16,
...
minetest.register_abm({
	nodenames = {"fire:basic_flame", "fire:permanent_flame","fire:blue_flame"},
