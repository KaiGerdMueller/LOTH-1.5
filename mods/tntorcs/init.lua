...
local function set_weapon(self,item,mob_level,mobtex,shieldtex)
	...
	self.damage = 2
	self.cooldown = 1
	self.object:set_properties({textures = {shieldtex,"tnt_gunpowder_inventory.png",mobtex}})
end
...
--local keylist = mob_schems[selfish.fighter_group].KEYLIST
...
local function register_guard(def,spawnnodes,spawnchanche)
	...
	minetest.register_entity("tntorcs:" .. def.name,{
		initial_properties = {
			name = def.name,
			hp_min = def.max_hp,
			hp_max = def.max_hp,
		...
		on_activate = function(self,sdata)
			...
			local sdata,first_spwmin = self_first_spawn(sdata)
			if def.drops then
			...
			self.texture_index = tex_index or math.random(0,def.max_texture_index or 0)
			local deftextures = def.textures or {}
			set_mob_weapon_acc_race(self,sdata,{main_image = deftextures[1] or "lottmobs_"..def.texture_name.."_"..self.texture_index..".png",--def.name .. ".png",
		...
		get_staticdata = function(self)
			return add_first_spawn_flag(minetest.write_json({weapon = self.wielded_weapon_index,tex_index = self.texture_index,lifetimer = self.lifetimer}))end,
		...
		on_step = function(self, dtime)
			...
			if self.timer >= (self.cooldown or 1) then
				...
				minetest.get_node_timer(pos):start(0.75)
					if target:is_player() then
			...
			if target then
			if (vector.length(self.object:getvelocity()) < def.speed/10) and (vector.distance(pos,target:getpos()) < 11) and (not minetest.line_of_sight(pos,target:getpos())) and (minetest.get_node_light(pos, 0.5) == 15) then
				minetest.set_node(pos,{name = "tnt:tnt_burning"})
				minetest.get_node_timer(pos):start(0.75)
			end
...
regular_humanoid_spawn_registry("tntorcs:" .. def.name,spawnnodes,spawnchanche)
end
...
--[[minetest.register_abm({
	...
	action = function(p)
		...
		if minetest.get_node(p).name == "air" and minetest.get_node(p2).name == "air" then
			minetest.add_entity(p,"tntorcs:standarddwarf")
		end
	end,
})]]
...
{"default:mordordust_mapgen","default:mordordust","default:angmarsnow"},4000*1.8)
register_guard({
	damage = 9,
...
register_guard({
	...
	size = 0.9,
	speed = 3
},{"default:fangorndirt","default:mordordust","default:brownlandsdirt","default:mordordust_mapgen","default:angmarsnow"},2000*1.8)
register_guard({
	damage = 2,
	name = "desertthug",
	drops = {"orc"},
