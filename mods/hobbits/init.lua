...
local function get_nearest_enemy(self,pos,radius,fucking_punchtest)
	...
	for _,entity in ipairs(minetest.get_objects_inside_radius(pos,radius)) do
		...
		if --[[(luaent and (not luaent.hudwel)) or]] (entity:is_player() and hudwelshlaw[entity:get_player_name()]  and hudwelshlaw[entity:get_player_name()] > 0) then
...
function stomp(sf,size,jh)
	local pos = sf.object:getpos()
	pos.y = pos.y+(size or 1)+(jh or 1.5)
	local n = minetest.registered_nodes[minetest.get_node(pos).name]
	if n and not n.walkable then
		pos.y = pos.y-size
		sf.object:setpos(pos)
		return true
	else
		return false
	end
...
local function register_guard(def,spawnnodes,spawnchanche)
	...
	--end
	local defbox = 0.3
	minetest.register_entity("hobbits:" .. def.name,{
		initial_properties = {
			name = def.name,
			hp_min = def.max_hp,
			hp_max = def.max_hp,
			...
			visual = "mesh",
			mesh = "character.b3d",
			textures = {"doors_blank.png"},--def.name .. ".png"},
			collisionbox = {-defbox, -def.size, -defbox, defbox, def.size, defbox},
			physical = true
		},
		-- ON ACTIVATE --
		on_activate = function(self,sdata)
			self.texture_index = tonumber(sdata) or math.random(0,5)
			self.object:set_properties({textures = {"lottmobs_hobbit_"..self.texture_index..".png"}})
