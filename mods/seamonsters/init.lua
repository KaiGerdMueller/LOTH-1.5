...
local function get_nearest_enemy(pos,radius,fucking_punchtest)
	...
	for _,entity in ipairs(minetest.get_objects_inside_radius(pos,radius+1)) do
		...
		if (luaent and (not luaent.seamonster)) or entity:is_player() then
		if luaent and luaent.boats_boat and fucking_punchtest then
			entity:remove()
		else
...
function register_jellyfish_advanced(name, def)
	...
	minetest.register_entity("seamonsters:" .. name,{
		initial_properties = {
			name = "seamonsters:" .. name,
			hp_max = def.max_hp,
			visual_size = {x = def.visualsize*5, y = def.visualsize*5, z = def.visualsize*5},
			visual = "mesh",
		...
		on_step = function(self, dtime)
			...
			if self.timer >= 1 then
				self.timer = 0
				local obj = get_nearest_enemy(pos,3,true)
				if obj then
			...
			if wian == "air" then
				...
				if (wian == "default:water_source") or (wian == "default:water_flowing") then
				self.object:setpos(self.last_watered_pos)
				else
				self.object:set_hp(0)
				end
			elseif (wian == "default:water_source") or (wian == "default:water_flowing") then
	...
	register_spawn("seamonsters:" .. name, "default:water_source",def.chanche)
end
...
register_jellyfish({
	...
	textures = {"seaserpent_texture.png"},
	size = 1.5*seamonster_size_const,
	visual_size = seamonster_size_const,
	heigth = seamonster_size_const,
	max_hp = 600,
