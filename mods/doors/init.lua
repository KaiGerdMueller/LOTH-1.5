...
function _doors.door_toggle(pos, clicker)
	...
	--if state == "" then
	--	db({"AAAGGGRRRESSIIIOOOONNNEEEENNN!!!!!!!!"})
	--end
	if state == "" then
	...
	if clicker then--not minetest.check_player_privs(clicker, "protection_bypass") then
...
function doors.register(name, def)
	...
	-- DEPRECATED BY KGM;BREAKING SOME MT VERSIONS!!!
...
for _,name in pairs(by_register_tree_components_registered_wood_types) do
doors.register_trapdoor("doors:trapdoor_" .. name .. "wood", {
	...
	tile_side = "doors_trapdoor_side_blackwhite.png^" ..  name .. "_tree_overlay.png",
	groups = {snappy=1, choppy=2, oddly_breakable_by_hand=2, flammable=2, door=1},
})end
